...
class App extends Component {
  ...
  getTokenBalance = () => {
    if(this.state.network === 'Mainnet') {
      ...
      this.xhr(
        `https://api-rinkeby.etherscan.io/api?module=account&action=tokenbalance&contractaddress=0x6b5face539505bac2b3bf5746070445f5fda4afe&apiKey=2SDT7WUWQDZ8EJZWHP6QUPEZAX8ZE7VPV2&address=${this.state.account}`, 
      (res) => {
        const data = JSON.parse(res);
        const balance = data.result / 10**18;
        if(balance > 0) {
